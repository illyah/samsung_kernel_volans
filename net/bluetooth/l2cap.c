...
#define HCI_BROADCOM_QOS_CMD 0xFC57
...
static void __l2cap_sock_close(struct sock *sk, int reason)
{
	...
	case BT_CONNECT2:
		if (sk->sk_type == SOCK_SEQPACKET) {
			...
			req.scid = cpu_to_le16(l2cap_pi(sk)->scid);
			/*@LDK add the code to check wheher the conn structure is null*/
			if(conn!=NULL){
...
/*@LDK we will not execute link_key request to resolve BENZ carkit connection request*/
#if 0
...
static inline int l2cap_connect_rsp(struct l2cap_conn *conn, struct l2cap_cmd_hdr *cmd, u8 *data)
{
	...
	case L2CAP_CR_SUCCESS:
		...
		/*@LDK we will patch to TI firmware to gurantee the A2DP packet*/
		if(l2cap_pi(sk)->psm == 25)
		{
			printk("BROADCOM A2DP start!!!!\n");
		    struct hci_cp_broadcom_cmd cp;
			cp.handle = cpu_to_le16(conn->hcon->handle);

			hci_send_cmd(conn->hcon->hdev, HCI_BROADCOM_QOS_CMD, sizeof(cp), &cp);
		}
