...
/* Touch Screen */
static struct resource sec_ts_resource[] = {
	[0] = {
		.start = S3C_PA_ADC,
		.end   = S3C_PA_ADC + SZ_4K - 1,
		.flags = IORESOURCE_MEM,
...
struct platform_device sec_device_ts = {
	.name		  = "sec-ts",
	.id		  = -1,
	.num_resources	  = ARRAY_SIZE(sec_ts_resource),
	.resource	  = sec_ts_resource,
};

static struct resource sec_keypad_resource[] = {
	[0] = {
		.start = S3C_PA_KEYPAD,
		.end   = S3C_PA_KEYPAD+ S3C_SZ_KEYPAD - 1,
		.flags = IORESOURCE_MEM,
...
struct platform_device sec_device_keypad = {
	.name		  = "sec-keypad",
	.id		  = -1,
	.num_resources	  = ARRAY_SIZE(sec_keypad_resource),
	.resource	  = sec_keypad_resource,
};
EXPORT_SYMBOL(sec_device_keypad);
...
void __init samspi_set_info(unsigned id, struct sam_spi_mstr_info *info)
{
	if(id == 0)
	   s3c_device_spi0.dev.platform_data = info;
	else if(id == 1)
	   s3c_device_spi1.dev.platform_data = info;
}
