...
void __init_or_cpufreq s3c6400_setup_clocks(void)
{
	...
	tmp |= (0x1 << 26);						/* LCD clock source = dout_mpll */
	tmp |= (0x1 << 18) | (0x1 << 20);		/* MMC0,MMC1 clock source = dout_mpll */
	tmp |= (0x1 << 13);						/* UART = dout_mpll */
	__raw_writel(tmp, S3C_CLK_SRC);

	tmp = __raw_readl(S3C_CLK_DIV1);
	tmp |= (0x5 << 4) | (0x5 << 0);			/* MMC bus clock = 22.1MHz (133 / 6) */
	__raw_writel(tmp, S3C_CLK_DIV1);

	tmp = __raw_readl(S3C_CLK_DIV2);
	tmp &= ~(0xf << 16);
	__raw_writel(tmp, S3C_CLK_DIV2);
