...
void s3c_fimc0_cfg_gpio(struct platform_device *dev)
{
	...
	s3c_gpio_cfgpin(S3C64XX_GPF(0), S3C64XX_GPF0_CAMIF_CLK);      // MCLK
	s3c_gpio_cfgpin(S3C64XX_GPF(1), S3C64XX_GPF1_CAMIF_HREF);
	s3c_gpio_cfgpin(S3C64XX_GPF(2), S3C64XX_GPF2_CAMIF_PCLK);     // PCLK
	...
	for (i = 0; i < 12; i++)
		s3c_gpio_setpull(S3C64XX_GPF(i), S3C_GPIO_PULL_NONE);


//	s3c_gpio_setpull(S3C64XX_GPB(4), S3C_GPIO_PULL_UP);

	s3c_gpio_cfgpin(GPIO_CAM_3M_nRST, S3C_GPIO_SFN(GPIO_CAM_3M_nRST_AF));
	s3c_gpio_setpull(GPIO_CAM_3M_nRST, S3C_GPIO_PULL_NONE);

	s3c_gpio_cfgpin(GPIO_CAM_EN, S3C_GPIO_SFN(GPIO_CAM_EN_AF));
	s3c_gpio_setpull(GPIO_CAM_EN, S3C_GPIO_PULL_NONE);

//	s3c_gpio_cfgpin(GPIO_CAM_3M_nSTBY, S3C_GPIO_SFN(GPIO_CAM_3M_nSTBY_AF));
//	s3c_gpio_setpull(GPIO_CAM_3M_nSTBY, S3C_GPIO_PULL_NONE);

	s3c_gpio_cfgpin(GPIO_CAM_CIF_nRST, S3C_GPIO_SFN(GPIO_CAM_CIF_nRST_AF));
	s3c_gpio_setpull(GPIO_CAM_CIF_nRST, S3C_GPIO_PULL_NONE);

	s3c_gpio_cfgpin(GPIO_CAM_CIF_nSTBY, S3C_GPIO_SFN(GPIO_CAM_CIF_nSTBY_AF));
	s3c_gpio_setpull(GPIO_CAM_CIF_nSTBY, S3C_GPIO_PULL_NONE);
#if 0	
	gpio_set_value(GPIO_CAM_3M_nSTBY, 0);
	gpio_set_value(GPIO_CAM_3M_nRST, 0);
	gpio_set_value(GPIO_CAM_EN, 0);
	gpio_set_value(GPIO_CAM_CIF_nSTBY, 1);
	gpio_set_value(GPIO_CAM_CIF_nRST, 0);
#endif
