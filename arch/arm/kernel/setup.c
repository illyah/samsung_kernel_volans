...
static void __init early_logging_mode(char **p) 
{ 
	unsigned int size = strlen(*p);
	unsigned long long ret = simple_strtoull (*p, (p+size), 0);
	 
	logging_mode = ret;
	printk(KERN_ERR"[%s] logging_mode = %lu\n", __FUNCTION__, logging_mode);
	return;
}
__early_param("logging_mode=", early_logging_mode);
