...
#define DRIVER_NAME "synaptics-rmi4"
...
static void touchscreen_hw_init(void)
{
                                                ...
                                                struct synaptics_rmi4_fn *rfi)
{
        ...
        if (retval != QUERY_LEN)
                ...
                pdata->sensor_max_x = 240; /*((data[6] & MASK_8BIT) << 0) |
                                                ((data[7] & MASK_4BIT) << 8);*/
                pdata->sensor_max_y = 400; /*((data[8] & MASK_5BIT) << 0) |
                                                ((data[9] & MASK_4BIT) << 8);*/
        }
        return retval;
}
        ...
        (struct i2c_client *client, const struct i2c_device_id *dev_id)
{
    ...
    nowplus_enable_touch_pins(1);
        ...
        retval = request_irq(platformdata->irq_number,
                                        synaptics_rmi4_irq,
