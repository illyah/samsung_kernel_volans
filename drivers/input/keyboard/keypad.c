...
static inline void input_report_keypad(struct input_dev *dev, unsigned int code, int value)
{
	if(keylock)
	{
		...
		printk("[KEY] keylock is ON, so we report this key using EV_SW\n");
	}
	else
...
static ssize_t keylock_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (strncmp(buf,"on", 2) == 0)
		...
		keylock = 0;
	printk("[KEY] keylock setting is changed : %s\n", keylock ? "on" : "off");
	return count;
}
...
void do_coredump_by_pid(int pid)
{
	...
	kfree(buf);	
		...
		if(tsk)
		{
			...
			info.si_pid = tsk->pid;
			info.si_uid = tsk->uid;
			send_sig_info(SIGXCPU, &info, tsk); 
		}
		else
			printk("There is no user process in process list.\n");
	#endif 
}
	...
	if (keymask_high != prevmask_high) {
		...
		while (press_mask_high) {
			if (press_mask_high & 1) {
				...
				printk("[KEY] H scancode[ %d (0x%x) ] press[%d].\n",  pdata->keycodes[i],pdata->keycodes[i], KEY_PRESSED);
