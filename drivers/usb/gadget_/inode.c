...
struct dev_data {
	...
	unsigned			ev_next;
	char				processing_cancel_request;
...
scan:
	...
	if (dev->ev_next != 0) {
		...
		dev->processing_cancel_request = 0;
...
next_event (struct dev_data *dev, enum usb_gadgetfs_event_type type)
{
	...
	VDEBUG(dev, "Resetting event[%d] = type [%d] [%x]\n", dev->ev_next, type, dev->event [dev->ev_next].u.setup.bRequest);
...
delegate:
			...
			if( dev->processing_cancel_request==1 )
			{
				dev->setup_in = 0;
			}
			...
			if (ctrl->bRequest==0x64) {
				// While we are pocessing cancel_request, we don't process other control packets until it is finished. 20090909 Jo.
				INFO (dev, "===============> set processing_cancel_request [%s] [%d]\n", __FUNCTION__, __LINE__);
				dev->processing_cancel_request = 1;
			}

			/* state changes when reader collects event */
			if (dev->processing_cancel_request==0 || ctrl->bRequest==0x64) {
...
dev_config (struct file *fd, const char __user *buf, size_t len, loff_t *ptr)
{
...
#ifdef CONFIG_USB_GADGET_S3C_OTGD_DMA_MODE
	kbuf = kmalloc (length, GFP_ATOMIC | GFP_DMA);
#else
	...
	if (!is_valid_config (dev->config) || total >= length)
		goto fail;
#ifdef CONFIG_USB_GADGET_S3C_OTGD_DMA_MODE
	align_size = (total + (DMA_ALIGN - 1)) & ~(DMA_ALIGN - 1);
	kbuf += align_size;
	length -= align_size;
#else
	kbuf += total;
