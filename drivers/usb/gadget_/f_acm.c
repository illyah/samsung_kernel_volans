...
//for using Qualcomm acm
#define USING_QualComm_WIN_DRV4ACM  0 

//for usging qualcomm patch submitted to google source
#define USING_QualComm_PATCH	0

//for using MCCI Samsung Mobile Modem Composite driver
#define USING_MCCI_WIN_DRV4ACM  1 
...
#if USING_QualComm_WIN_DRV4ACM
	static	struct usb_interface_assoc_descriptor acm_IAD __initdata = {
		.bLength =		sizeof(acm_IAD),
		.bDescriptorType = USB_DT_INTERFACE_ASSOCIATION,
//		.bFirstInterface = 0x02,
		.bInterfaceCount = 0x02,
#if USING_QualComm_PATCH 
		//the following is changed by Qualcomm patch
	 	//same with control interface
		.bFunctionClass =	USB_CLASS_COMM,
		.bFunctionSubClass =	USB_CDC_SUBCLASS_ACM,
		.bFunctionProtocol =	USB_CDC_ACM_PROTO_AT_V25TER,
#else
		//same with data interface
		.bFunctionClass  = USB_CLASS_CDC_DATA,
		.bFunctionSubClass = 0,
		.bFunctionProtocol = 0,
#endif		
	//	.iFunction = ,
...
#if USING_QualComm_WIN_DRV4ACM	
	[ACM_IAD_IDX].s = "CDC ACM Composite IAD",
#endif	
	{  /* ZEROES END LIST */ },
};
...
#if USING_QualComm_PATCH 
	acm_IAD.bFirstInterface = status;
#endif
