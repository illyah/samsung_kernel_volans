...
#define ACM_DEV_OFF			0
#define ACM_DEV_READY		1
#define ACM_DEV_OPENED		2
static int acm_dev_status=0;



static int modem_open(struct inode *inode, struct file *file)
{
	if(acm_dev_status == ACM_DEV_OPENED)
	{
		printk("ACM is busy.\n");
		return -EBUSY;
	}
...
void notify_control_line_state(u32 value)
{
	...
	if(acm_dev_status > ACM_DEV_READY)
	{
		printk("ACM : %s(%d) wakeup\n", __FUNCTION__,__LINE__);
...
int modem_register(void * data)
{
	...
	if (ret) {
		printk(KERN_ERR "DUN register failed, ret = %d\n", ret);
		return ret;
	}
...
void modem_unregister(void)
{
	...
	//printk(KERN_INFO "DUN is unregistered\n");
	acm_dev_status = ACM_DEV_OFF;
}
