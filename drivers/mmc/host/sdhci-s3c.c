...
static int __devinit sdhci_s3c_probe(struct platform_device *pdev)
{
	...
	if(pdev->id == 0)
	{
		...
		if (irq_cd < 0) {
			dev_err(dev, "no irq_cd specified\n");
			return irq_cd;
		}
	}
	else
	...
	if(gpio_get_value(GPIO_T_FLASH_DETECT))
	{
		...
		s3c_gpio_setpull(GPIO_T_FLASH_CLK, S3C_GPIO_PULL_DOWN);
	      	
		s3c_gpio_cfgpin(GPIO_T_FLASH_CMD, S3C_GPIO_INPUT);
		s3c_gpio_setpull(GPIO_T_FLASH_CMD, S3C_GPIO_PULL_DOWN);
	      
		s3c_gpio_cfgpin(GPIO_T_FLASH_D0, S3C_GPIO_INPUT);
		s3c_gpio_setpull(GPIO_T_FLASH_D0, S3C_GPIO_PULL_DOWN);

		s3c_gpio_cfgpin(GPIO_T_FLASH_D1, S3C_GPIO_INPUT);
		s3c_gpio_setpull(GPIO_T_FLASH_D1, S3C_GPIO_PULL_DOWN);

		s3c_gpio_cfgpin(GPIO_T_FLASH_D2, S3C_GPIO_INPUT);
		s3c_gpio_setpull(GPIO_T_FLASH_D2, S3C_GPIO_PULL_DOWN);
		
		s3c_gpio_cfgpin(GPIO_T_FLASH_D3, S3C_GPIO_INPUT);
		s3c_gpio_setpull(GPIO_T_FLASH_D3, S3C_GPIO_PULL_DOWN);
	}
#endif
