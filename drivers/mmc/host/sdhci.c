...
static void sdhci_tasklet_card(unsigned long param)
{
	...
	spin_lock_irqsave(&host->lock, flags);
#if defined(CONFIG_MACH_VOLANS)
	if ((!(readl(host->ioaddr + SDHCI_PRESENT_STATE) & SDHCI_CARD_PRESENT)) || !card_detected) {
#else
	if (!(readl(host->ioaddr + SDHCI_PRESENT_STATE) & SDHCI_CARD_PRESENT)) {
#endif
		if (host->mrq) {
...
static irqreturn_t sdhci_irq_cd(int irq, void *dev_id)
{
	...
	intmask = readl(S3C64XX_GPN_DATA);
	intmask &= 0x40;

	if ((!intmask && !card_detected))
	{
		gpio_set_value(GPIO_TF_EN, GPIO_LEVEL_HIGH);
		card_detected = 1;
		DBG("*** %s got card insert signal\n",
			mmc_hostname(host->mmc));
	}
	else if(intmask && card_detected)
	{
		gpio_set_value(GPIO_TF_EN, GPIO_LEVEL_LOW);
		card_detected = 0;
		DBG("*** %s got card remove signal\n",
			mmc_hostname(host->mmc));
	}
	else
	{
	...
	mdelay(1);
	intnum = irq - IRQ_EINT(0);
	__raw_writel((1 << intnum), S3C64XX_EINT0PEND);
...
int sdhci_add_host(struct sdhci_host *host)
{
	...
	if(host->id == 0)
	{
		ret = request_irq(host->irq_cd, sdhci_irq_cd, IRQF_DISABLED,
			mmc_hostname(mmc),host);
		if (ret)
			goto untasklet;
	}
