...
{
	printk("kxsd9_interrupt_handler start\n");
...
static void kxsd9_workqueue_func(struct work_struct *work)
{
	...
	printk("kxsd9_workqueue_func has been activated by kxsd9_timer_func()\n");
	...
	printk("\n");
	printk("[DBG] buf_read[0]=%d, buf_read[1]=%d\n", buf_read[0], buf_read[1]);
	printk("[DBG] buf_read[2]=%d, buf_read[3]=%d\n", buf_read[2], buf_read[3]);
	printk("[DBG] buf_read[4]=%d, buf_read[5]=%d\n\n", buf_read[4], buf_read[5]);
...
static int kxsd9_i2c_probe_found(struct i2c_adapter *adapter, int address, int kind)
{
	...
	strlcpy(new_client->name, "KXSD9", I2C_NAME_SIZE);
	...
	if(ret)
	{
		printk("kxsd9 misc device register failed\n");
		goto exit;
	}
