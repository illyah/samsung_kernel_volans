...
int s3cfb_disable_local_win0(struct s3cfb_global *ctrl)
{	
	unsigned int cfg;
	
	cfg = readl(ctrl->regs + S3C_WINCON(0));
	cfg &= ~S3C_WINCON_DATAPATH_LOCAL;
	writel(cfg, ctrl->regs + S3C_WINCON(0));	

	return 0;
}

void s3cfb_check_line_count100(struct s3cfb_global *ctrl)
{
   int timeout = 30 * 5300;
   int i = 0;
   int cnt;
   ...
   do {
	   cnt = (readl(ctrl->regs + S3C_VIDCON1) & 0x7ff0000)>>16;
	   if ((50 <= cnt) && (cnt <= 150))
		   break;
	   i++;
   } while (i < timeout);
   ...
   if (i == timeout) {
	   lcd_err( " line cnt doesn't reach 100 %d\n", cnt);
	   s3cfb_display_on(ctrl);
   }
}
...
int s3cfb_frame_on(struct s3cfb_global *ctrl)
{
	unsigned int cfg;
	...
	lcd_dbg(" current frame display is on\n");
...
int s3cfb_frame_off(struct s3cfb_global *ctrl)
{
	...
	lcd_dbg(" current frame display is off\n");
...
int s3cfb_set_clock(struct s3cfb_global *ctrl)
{
	...
	src_clk = clk_get_rate(ctrl->clock->parent);
	...
	lcd_dbg(" parent clock: %d, vclk: %d, vclk div: %d\n", src_clk, vclk, div);
...
int s3cfb_set_global_interrupt(struct s3cfb_global *ctrl, int enable)
{
	...
	if (enable) {
		lcd_dbg(" video interrupt is on\n");
		cfg |= (S3C_VIDINTCON0_INTFRMEN_ENABLE | \
			S3C_VIDINTCON0_INT_ENABLE);
	} else {
		lcd_dbg(" video interrupt is off\n");
...
int s3cfb_set_vsync_interrupt(struct s3cfb_global *ctrl, int enable)
{
	...
	if (enable) {
		lcd_dbg(" vsync interrupt is on\n");
		cfg |= S3C_VIDINTCON0_FRAMESEL0_VSYNC;
	} else {
		lcd_dbg(" vsync interrupt is off\n");
		cfg &= ~S3C_VIDINTCON0_FRAMESEL0_VSYNC;
	}
	...
	if (win->path == DATA_PATH_FIFO) {
		...
		case 16:
			if (var->transp.length == 1) {
				lcd_dbg(" [fb%d] bpp mode: A1-R5-G5-B5\n", id);
				cfg |= S3C_WINCON_BPPMODE_16BPP_A555;
			} else if (var->transp.length == 4) {
				lcd_dbg(" [fb%d] bpp mode: A4-R4-G4-B4\n", id);
				cfg |= S3C_WINCON_BPPMODE_16BPP_A444;
			} else {
				lcd_dbg(" [fb%d] bpp mode: R5-G6-B5\n", id);
		...
		case 32:
			if (var->transp.length == 0) {
				lcd_dbg(" [fb%d] bpp mode: R8-G8-B8\n", id);
				cfg |= S3C_WINCON_BPPMODE_24BPP_888;
			} else {
				lcd_dbg(" [fb%d] bpp mode: A4-R8-G8-B8\n", id);
...
int s3cfb_set_buffer_address(struct s3cfb_global *ctrl, int id)
{
	...
	lcd_dbg(" [fb%d] start_addr: 0x%08x, end_addr: 0x%08x\n", \
...
int s3cfb_set_alpha_blending(struct s3cfb_global *ctrl, int id)
{
	...
	if (id == 0) {
		lcd_err( "[fb%d] does not support alpha blending\n", id);
		return -EINVAL;
	}
	...
	if (alpha->mode == PIXEL_BLENDING) {
		lcd_dbg(" [fb%d] alpha mode: pixel blending\n", id);

		
		cfg |= (S3C_WINCON_BLD_PIXEL | S3C_WINCON_ALPHA1_SEL);
	} else {
		lcd_dbg(" [fb%d] alpha mode: plane %d blending\n", \
			id, alpha->channel);
...
int s3cfb_set_chroma_key(struct s3cfb_global *ctrl, int id)
{
	...
	if (id == 0) {
		lcd_err( " [fb%d] does not support chroma key\n", id);
		return -EINVAL;
	}
	...
	lcd_dbg(" [fb%d] chroma key: 0x%08x, %s\n", id, cfg, \
