...
static int __init s3cfb_map_video_memory(s3cfb_info_t *fbi)
{
	...
	if (fbi->map_cpu_f1) {
		...
		if(fbi->win_id == FB_WIN_ID)
		{
			printk("RGB888 logo size = %ld, FB1 size = %ld\n", sizeof(arr_samsung_boot_logo), fbi->map_size_f1);
			if( sizeof(arr_samsung_boot_logo) <= fbi->map_size_f1)
				memcpy(fbi->map_cpu_f1, arr_samsung_boot_logo, sizeof(arr_samsung_boot_logo));
			else
