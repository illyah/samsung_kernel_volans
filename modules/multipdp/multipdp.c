...
static int vnet_stop(struct net_device *net)
{
    ...
    if (vnet_start_xmit_flag) { 
        ...
        printk("[DPRAM] clear vnet_start_xmit_flag\n"); 
    }
	return 0;
}
...
static int vnet_start_xmit(struct sk_buff *skb, struct net_device *net)
{
    ...
    if (vnet_start_xmit_flag != 0) {
        if (vnet_start_xmit_count > 100000) {
            vnet_start_xmit_flag = 0;
            printk("[ONEDRAM] vnet_start_xmit_count exceed.. clear vnet_start_xmit_flag \n");
        }
        printk("vnet_start_xmit() return -EAGAIN \n");
...
static int vs_read(struct pdp_info *dev, size_t len, int vs_id)
{
	...
	if (dev) {
		...
		if (len > MAX_PDP_DATA_LEN) {	// RF cal data?
            ...
            printk("[CAL] multipdp_thread request read size : %d readed size %d, count : %d\n",len ,size,count);

            if ((dev->id == 25 && !fp_vsROUTER) || (dev->id == 1 && !fp_vsCSD) || (dev->id == 5 && !fp_vsGPS) || (dev->id == 8 && !fp_vsRFS)){
			...
			if (retval > 0 && dev->vs_dev.tty != NULL) {
				tty_insert_flip_string(dev->vs_dev.tty, pdp_rx_buf, retval);
				tty_flip_buffer_push(dev->vs_dev.tty);
			}
#endif             
		}
...
static inline int pdp_add_dev(struct pdp_info *dev)
{
    ...
    printk("[MULTIPDP] pdp_add_dev() Error ..%d There is no space to make\n",dev->id);
	return -ENOSPC;
}
...
static int pdp_mux(struct pdp_info *dev, const void *data, size_t len   )
{
    ...
    #if 1
        if(pdp_tx_flag){
		if (dev->type == DEV_TYPE_NET || ((dev->id == 25)&&(!pdp_atcmd_flag)))	
        		return -EAGAIN;
        }
    #endif
...
static int pdp_demux(void)
{
	...
	if (ret < 0) {
        printk("[DPRAM] pdp_demux() dpram_read ret : %d\n",ret);
        mutex_unlock(&pdp_lock);
		return ret;
	}
	...
	if (ret < 0 || ch != 0x7e) {
        printk("[DPRAM] STOP BIT Value : %x, ret : %d\n",ch,ret);
        mutex_unlock(&pdp_lock);
		return ret;
	}
    mutex_unlock(&pdp_lock);
...
static int pdp_activate(pdp_arg_t *pdp_arg, unsigned type, unsigned flags)
{
	...
	printk("[MULTIPDP] pdp_activate() id: %d\n", pdp_arg->id);
	//DPRINTK(2, "id: %d\n", pdp_arg->id);
...
static int pdp_deactivate(pdp_arg_t *pdp_arg, int force)
{
	...
	printk("[MULTIPDP] pdp_deactivate() id: %d\n", pdp_arg->id);
	...
	if (dev == NULL) {
		printk("[MULTIPDP]n ot found id: %u\n", pdp_arg->id);
		mutex_unlock(&pdp_lock);
		return -EINVAL;
	}
    ...
    printk("[MULTIPDP] pdp_deactivate() OK");
	kfree(dev);
			      ...
			      unsigned int cmd, unsigned long arg)
{
    ...
    case HN_PDP_TXSTART:
    	pdp_tx_flag = 0;
    	printk("[MULTIPDP] tx_start.\n");
	    return 0;
			
	case HN_PDP_TXSTOP:
		pdp_tx_flag = 1;
		printk("[MULTIPDP] tx_stop.\n");
		return 0;
...
static int __init multipdp_init(void)
{
	...
	pdp_arg_t pdp_args[5] = {
		{ .id = 1, .ifname = "ttyCSD" },
		{ .id = 8, .ifname = "ttyRFS" },
		{ .id = 25, .ifname = "ttyROUTER" },
		{ .id = 5, .ifname = "ttyGPS" },
		{ .id = 6, .ifname = "ttyXtraGPS" },
	};

	/*
	pdp_arg_t pdp_arg = { .id = 1, .ifname = "ttyCSD", };
	pdp_arg_t efs_arg = { .id = 8, .ifname = "ttyRFS", };
	*/

    //prx_buf = kzalloc(15000, GFP_ATOMIC);
    prx_buf = vmalloc(BUFFER_SIZE_FOR_CAL);
    if (prx_buf == NULL) {
        printk("[CAL] Error..");
        printk("[CAL] ERROR.. cat't alloc memory prx_buf..\n");
