...
static void domain_pm_callback(struct work_struct *ignored);

static DECLARE_DELAYED_WORK(domain_pm_work, domain_pm_callback);
static DEFINE_MUTEX(domain_pm_lock);

atomic_t pm_critical_section = ATOMIC_INIT(0);
...
#define SHOW_3D_CHUNK_MEMORY_STATUS
#define TN_LIMO_SUPPORT

#ifdef TN_LIMO_SUPPORT
#define NEW_RESERVED_MEM_ADDR_PHY                (unsigned int)s3c_get_media_memory(S3C_MDEV_G3D)
#define NEW_RESERVED_MEM_SIZE                       (unsigned int)s3c_get_media_memsize(S3C_MDEV_G3D)
...
{	
	unsigned long physicAddr = (NEW_RESERVED_MEM_ADDR_PHY + NEW_RESERVED_MEM_SIZE) - (G3D_CHUNK_SIZE * (ithMem+ 1));
...
#ifdef SHOW_3D_CHUNK_MEMORY_STATUS
	#define S3C_3D_SHOW_MEM_STATUS		_IO(G3D_IOCTL_MAGIC, 316)
#endif
...
static int clk_g3d_count = 0;
static inline void clk_g3d_enable(void)
{
	if (!clk_g3d_count)
...
static inline void clk_g3d_disable(void)
{
	if (clk_g3d_count == 1)
...
static void domain_pm_callback(struct work_struct *ignored)
{
	mutex_lock(&domain_pm_lock);

	if (atomic_read(&pm_critical_section))
		schedule_delayed_work(&domain_pm_work, HZ);
	...
	unsigned int loop_i;
 	...
 	for(loop_i = 0; loop_i < G3D_CHUNK_NUM; loop_i++ ) {
		if( g3d_bootm[loop_i].in_used == G3D_CHUCNK_AVALIABLE ) {
			...
			g3d_bootm[loop_i].cur_pid = task_pid_nr(current);
...
{
	long timeout;
	u32 val,loop_a=0, allocated_chunk=0,free_chunk=0;
	...
	case WAIT_FOR_FLUSH:
		...
		while(1) {
			timeout=wait_event_interruptible_timeout(waitq, (interrupt_already_recevied>0),3*HZ);
			if (!timeout) 
			{
#ifdef SHOW_3D_CHUNK_MEMORY_STATUS
				printk("\ntimeout_3dm_status [ ");
				for(loop_a = 0; loop_a < G3D_CHUNK_NUM; loop_a++ ){
					if( g3d_bootm[loop_a].in_used == G3D_CHUCNK_AVALIABLE){
						free_chunk++;
						printk("F ");
					}
					else{
						allocated_chunk++;
						printk("%d ",g3d_bootm[loop_a].cur_pid);
					}
				}
				printk("] Alloc=%2d Free=%2d \n",allocated_chunk,free_chunk);
#endif
				break;
			}
...
#ifdef CONFIG_DOMAIN_GATING
		mutex_lock(&domain_pm_lock);
		atomic_inc(&pm_critical_section);
...
#ifdef CONFIG_DOMAIN_GATING
		mutex_lock(&domain_pm_lock);
		atomic_dec(&pm_critical_section);
		mutex_unlock(&domain_pm_lock);

		cancel_delayed_work(&domain_pm_work);
		schedule_delayed_work(&domain_pm_work, HZ);
#endif	/* CONFIG_DOMAIN_GATING */
		break;
...
int s3c_g3d_probe(struct platform_device *pdev)
{
	...
	if (g3d_clock == NULL) {
		printk(KERN_ERR PFX "failed to find hclk_g3d clock source\n");
		return -ENOENT;
	}
...
static char banner[] __initdata = KERN_INFO "S3C G3D Driver, (c) 2007-2009 Samsung Electronics, 2009.09.08\n";
